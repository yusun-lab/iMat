...
const Cart = () => {
  const { cartItems, foodList, deleteFromCart, getTotalCartAmount, url } =
  ...
  return (
    <div className="cart">
      <div className="cart-items">
        <div className="cart-items-title cart-items-grid">
          <p>Items</p>
          <p>Title</p>
          <p>Price</p>
          <p>Quantity</p>
        ...
        {foodList.map((item) => {
          if (cartItems[item._id] > 0) {
            return (
              <div key={item._id}>
                <div className="cart-items-item cart-items-grid">
                  <img src={url + "/images/" + item.image} alt="food image" />
                  <p>{item.name}</p>
                  <p>SEK {item.price}</p>
                  <p>{cartItems[item._id]}</p>
                  <p>SEK {item.price * cartItems[item._id]}</p>
